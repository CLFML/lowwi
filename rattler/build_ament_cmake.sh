...
# Set SP_DIR manually if not set
if [[ -z "${SP_DIR:-}" ]]; then
  SP_DIR="$($PREFIX/bin/python -c 'import site; print(site.getsitepackages()[0])')"
  export SP_DIR
fi
# Ensure both SP_DIR and PREFIX are set
if [[ -z "${SP_DIR:-}" ]]; then
  echo "Error: SP_DIR is not set" >&2
  exit 1
fi

if [[ -z "${PREFIX:-}" ]]; then
  echo "Error: PREFIX is not set" >&2
  exit 1
fi

# Compute relative path from SP_DIR to PREFIX
...
export RATTLER_BUILD_SKIP_RELINKING=1
...
$CMAKE_GEN \
    ...
    -DCMAKE_C_COMPILER="$BUILD_PREFIX/bin/gcc" \
    -DCMAKE_CXX_COMPILER="$BUILD_PREFIX/bin/g++" \
    -DCMAKE_INSTALL_PREFIX=$PREFIX \
    -DCMAKE_PREFIX_PATH=$PREFIX \
    -DAMENT_PREFIX_PATH=$PREFIX \
    -DCMAKE_INSTALL_LIBDIR=lib \
    -DCLFML_LOWWI_ROS2_PACKAGE_BUILD=ON \
